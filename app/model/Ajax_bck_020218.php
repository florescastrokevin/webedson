...
class Ajax{
	...
	public function updatePasswordCuentaAjax(){
		...
		$pass_actual = str_replace("'",'',comillas_inteligentes(encriptar($_GET['password_actual'])));
		
		//echo 'PASS ACTUAL : '.$pass_actual . 'PASS BDD : ' . $row['password_cliente'];
		
		if( $pass_actual == $row['password_cliente'] ){
		   $query = new Consulta("UPDATE clientes SET password_cliente=".comillas_inteligentes(encriptar($_GET['password_nuevo']))." 				
	...
	public function updateCuentaAjax(){
		...
		$query = new Consulta("UPDATE clientes SET  nombre_cliente = ".comillas_inteligentes($_GET['nombres'])." ,
                                        apellidos_cliente = ".comillas_inteligentes($_GET['apellidos'])." , 
                                        telefono_cliente = ".comillas_inteligentes($_GET['telefono'])." , 
                                        email_cliente = ".comillas_inteligentes($_GET['email'])." , 
	...
	public function accesoAjax(){
		...
		if($query->NumeroRegistros() > 0 && $prods > 0){
		...
		}else if( $query->NumeroRegistros() > 0 && $prods == 0){
	...
	public function listCestaAjax(){
        ...
        <!-- INICIO CARRITO-->
            ...
            <div id="loader_car"></div>
            <div id="countCar"><?php echo $total_c;?></div><div id="tipocambio">T.C. S/. <?php echo TIPO_CAMBIO?></div>
            ...
            <div id="boxCar">
                <ul>
                    ...
                    <li>
                         <a href="<?php echo str_replace(" ","-",$producto->__get("_nombre"))?>">
                            <div class="wrap_imgcar">
                                &nbsp;<?php if ( $imagenes[0]['thumbnail']!="" ) {?>
                            <img src="<?php echo _catalogo_ . $imagenes[0]['thumbnail']?>">
                            <?php }else{?>
                            <img src="<?php echo _imgs_ . "not_image_disponible_thumb.jpg"?>">	
                            <?php }?>
                            </div>
                            <div class="nameCar"><?php echo $producto->__get("_nombre")?> <br /> <span class="cant">Cantidad: <?php echo $content[$key]['cantidad'];?></span>
                            
                            
                            <?php if(is_array($complementos) && count($complementos)>0 ): ?>
                            
                                <div class="prod_complemento">                            
                                    <ul>
                                        <?php   
                            ...
                            <div class="priceCar">$<?php echo number_format(($precio + $preciocmp) * $content[$key]['cantidad'],2)?></div>
                        </a>
                    </li>
                     <?php
                    }
                ...
                <div id="subtotal">Subtotal: <span>$<?php echo number_format($carrito->getAmount(),2);?></span></div>
                <a id="btn_vercarrito" href="<?php echo _url_web_ ?>seccion/cesta"></a>
            </div>
            ...
            <div id="boxCar">
                <ul>
                    <li class="no_items">No hay productos en tu carrito</li>
                </ul>                
            </div>    
            <?php }?>
    ...
    public function addFechaEspecialDestinatarioAjax(){
		$query = new Consulta("INSERT INTO fechas_especiales VALUES ('','".$_GET['id_destinatario']."','".$_GET['id_tipo_destinatario']."','".$_GET['id_ocasion']."','".$_GET['fecha_especial']."')");
	}

	public function newClienteDestinatarioAjax(){
		//self::addClienteDestinatarioAjax();
		list($a,$b,$c)= explode("/",$_GET['fecha-especial']); // 13/02/2018
		...
		$fechas_especiales = Cliente::getFechasEspecialesClientes($_GET['id_cliente']);
        $i=1;
        if (is_array($fechas_especiales) && count($fechas_especiales)>0) {
        foreach ($fechas_especiales as $key => $fecha_especial) { ?>
            <tr class="info">
                <td ><?php echo $i ?></td>
                <td ><?php echo $fecha_especial['nombre_destinatario'] ?></td> 
                <td ><?php echo $fecha_especial['apellidos_destinatarios'] ?></td> 
                ...
                <td ><?php echo fecha_long($fecha_especial['fecha_especial']) ?></td>                                                         
            </tr> 
        <?php $i++; } } ?>  
		<?php
	}	
	...
	public function nuevoClienteDestinatarioAjax(){
		...
		list($a,$b,$c)= explode("/",$_GET['fecha-especial']); // 13/02/2018
		$fecha_especial = $c.'-'.$b.'-'.$a;
		$query = new Consulta("INSERT INTO fechas_especiales VALUES ('','".$id_destinatario."','".$_GET['id_tipo_destinatario']."','".$_GET['id_ocasion']."','".$fecha_especial."')");
