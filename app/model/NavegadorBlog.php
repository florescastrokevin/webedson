...
class NavegadorBlog{
	...
	function bucleCatTrail($id_cat = 0, $id_prod = 0, $id_ec = 0){
		...
		for($x = 0; $x < 10; $x++){ 
			...
			if($id_cat > 0 ){
				...
				$sql   = "	SELECT * FROM categorias_blog WHERE  id_categoria_blog = '".$id_cat."'";
				...
				$id_cat = $row['id_parent_categoria_blog'];

				$id 	= $row['id_categoria_blog']; 

				$nombre = $row['titulo_categoria_blog']; 				
		...
		if($id_prod > 0 && $id_ec == 0){
			...
			$this->ruta[$rx] = array(
									...
									'url'	=>	'publicaciones.php?id='.$producto->__get('_id'),

									'nombre'=>  $producto->__get('_titulo') );			
		...
		if($id_ec > 0){
			...
			$this->ruta[$rx] = array(
				...
				'url'	=>	'post.php?id='.$obj_c->__get('_id'),
	...
	function display($id_actual=0){?>
		...
		<a href="publicaciones.php?cat=0">Categorias Blog</a><?php 
