...
class Listado{
	...
	function simple(Consulta $query, $url="", $sub_url="", $options = TRUE){
		$return="";
		
		$colspan=$query->NumeroCampos();
		$return.= "
		<div id='content-area'>			
		<table class='listado' cellpading='0' cellspacing='0' >\n";	
		...
		for ($i = 1; $i < $query->NumeroCampos(); $i++){
			...
			$return.= "
                            <th class='titulo'><a href='". $_SERVER['PHP_SELF']."?orden=".$query->nombrecampo($i)."'>".ucwords($name)."</a></th>\n";
		}
		if($options){
		$return.= "<th class='titulo' align='center' width='80'>Opciones</th>
                        </tr>
                    </thead> \n";
		}
                $return.= "
		...
		while ($row = $query->verRegistros()) {
			...
			$return.= "
                        <tr "; if($x==0){ $return.= "class=row";}else{ $return.= "class='row odl'";} $return.= "> \n";
			...
			for ($i = 1; $i < $query->NumeroCampos(); $i++){
				...
				$return	.= "<td align='center'>
					<a href='#' onClick=mantenimiento('".$url."',".$id.",'edit') title=Editar class='tooltip'><img src='"._admin_."edit.png' ></a> &nbsp; 
					<a href='#' onClick=mantenimiento('".$url."',".$id.",'delete') title=Eliminar class='tooltip'> <img src='"._admin_."delete.png' ></a>&nbsp;";
					if(!empty($sub_url)){
	...
	/*Muestra un listado */
	public static function VerListado(Consulta $query , $url="", $sub_url="", $preview=""){
		$return="";
		$colspan = $query->NumeroCampos();
		$return.= "
				<div id='content-area'>
