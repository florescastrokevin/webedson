...
class Producto{
	...
	private $_id, $_idioma, $_categoria, $_nombre, $_descripcion_corta, $_precio_producto, $_imagenes , $_destacado , $_relacionados, $_tags, $_estado, $_tags_id, $_tags_nombre ,$_ocasiones, $_ocasiones_id , $_destinatarios , $_destinatarios_id , $_expresiones , $_expresiones_id , $_tipos , $_is_complemento ,$_tipos_id, $_insumos, $_id_insumos , $_dscto , $_precio_old , $_atributos = array() , $_complementos = array() , $_stock, $_filtros = array(), $_url;
	public function __construct($id = 0, Idioma $idioma = Null){
		...
		if($this->_id > 0){
			...
			$sql_filtros = "SELECT * FROM productos_filtros WHERE id_producto = '".$this->_id."' ";
			$query_filtros = new Consulta($sql_filtros);
			if ($query_filtros->NumeroRegistros()>0) {
				while ($row_filtro = $query_filtros->VerRegistro()) {
					$this->_filtros[]=$row_filtro['id_filtro'];
				}
			}
			
			
			$sql_imgs = " SELECT * FROM productos_imagenes WHERE id_producto = '".$this->_id."' ORDER BY orden_producto_imagen ASC";
...
WHERE id_producto = '".$this->_id."'");
			if($queryi->NumeroRegistros() > 0){
                            while($rowi = $queryi->VerRegistro()){
                                $this->_insumos[$rowi['id_insumo']] = array(
                                    $rowi['nombre_insumo'],
                                    $rowi['cantidad']
                                );
                            }
